...
**Статус:** IN PROGRESS (реализовано локально, ожидает подтверждения пользователя;
не закоммичено, не опубликовано)
...
**Реализовано локально (2026-09-17), не закоммичено — ожидает
...
- Не закоммичено (по постановке — ждать подтверждения).
- После подтверждения: `feat(TASK_054): …`, затем обновить
  `docs/PROJECT_STATUS.md`, `docs/ROADMAP.md`, `CHANGELOG.md`,
  `docs(TASK_054): …`, деплой, проверка `finance-v186` на production.
