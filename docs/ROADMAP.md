...
19. **TASK_017_PREMIUM_NAVIGATION_DRAWER** — `DONE`. Полная визуальная
    ...
    изменений — сервисы/логика не затронуты). Реальная проверка на
    iPhone Safari/PWA не выполнена — инструмент iOS-симулятора вернул
    ошибку в этой сессии, рекомендуется пользователю после деплоя. См.
