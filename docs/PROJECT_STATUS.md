...
`TASK_015_ADVANCED_TRANSACTION_METADATA` — статус `PLANNED`: TASK-файл
оформлен, **план и спецификация утверждены пользователем 2026-07-27**
(все семь спорных решений закрыты), реализация ещё не начата.
`TASK_002A` (полноценный визуально заметный Liquid Glass) остаётся
отложенной.

## TASK_015 — Advanced Transaction Metadata (PLANNED)
...
**План утверждён пользователем 2026-07-27.** Все семь спорных решений
закрыты и перенесены в спецификацию; открытых блокирующих вопросов нет.
Отложены как отдельные будущие задачи: нормализация диакритики в поиске
(`Gijón` ↔ `gijon`) и перенос метаданных в схему `reminders`.
Реализация ещё не начата — см.
...
`TASK_015_ADVANCED_TRANSACTION_METADATA` — статус `PLANNED`, план
утверждён; следующий шаг — перевод задачи в `IN PROGRESS` и реализация
по пошаговому плану из её TASK-файла.
