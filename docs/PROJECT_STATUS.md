...
- **Задача:** [`TASK_022_BUDGETS_APPLE_REDESIGN`](tasks/TASK_022_BUDGETS_APPLE_REDESIGN.md)
- **Статус:** `DONE` (709 passed, 0 failed по всем 10 файлам тестов —
  675 существующих без регрессий + 34 новых; проверено в браузере на
  мобильной ширине 375×812 — герой-карточка «Осталось в бюджете» с тем
  же градиентом/тенью, что «Общий капитал» на «Счетах», обычный/
  почти-исчерпанный/перерасход бюджета, переключение месяца,
  создание/редактирование/удаление бюджета, пустое состояние, светлая
  и тёмная тема, консоль без ошибок)
- Ранее завершённые задачи: [`TASK_021_PROFILE_SCREEN_APPLE_REDESIGN`](tasks/TASK_021_PROFILE_SCREEN_APPLE_REDESIGN.md),
...
Нет. `TASK_022_BUDGETS_APPLE_REDESIGN` — статус `DONE`, полный
...
`TASK_022_BUDGETS_APPLE_REDESIGN` завершена; следующая задача не
определена — ждёт постановки пользователем. Production-верификация
`finance-v165` — см. §10 `TASK_022_BUDGETS_APPLE_REDESIGN.md`.
