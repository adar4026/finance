...
- **Задача:** [`TASK_021_PROFILE_SCREEN_APPLE_REDESIGN`](tasks/TASK_021_PROFILE_SCREEN_APPLE_REDESIGN.md)
- **Статус:** `DONE` (675 passed, 0 failed по всем 9 файлам тестов —
  595 существующих без регрессий + 80 новых; проверено в браузере на
  мобильной ширине 375×812 — шторка без «Безопасности», переход в
  профиль, редактирование имени, сквозная смена фото, переход в
  «Безопасность» и возврат ровно на профиль, кнопка «Назад» и Escape,
  светлая и тёмная тема, консоль без ошибок в том числе после полной
  перезагрузки)
- Ранее завершённые задачи: [`TASK_020_ACCOUNTS_APPLE_REDESIGN`](tasks/TASK_020_ACCOUNTS_APPLE_REDESIGN.md),
...
Нет. `TASK_021_PROFILE_SCREEN_APPLE_REDESIGN` — статус `DONE`, личный
...
`TASK_021_PROFILE_SCREEN_APPLE_REDESIGN` завершена; следующая задача не
определена — ждёт постановки пользователем. Production-верификация
`finance-v164` — см. §10 `TASK_021_PROFILE_SCREEN_APPLE_REDESIGN.md`.
